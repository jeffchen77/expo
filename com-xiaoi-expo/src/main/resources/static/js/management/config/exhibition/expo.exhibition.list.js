$(function() {
    ...
    layui.table.render({
        elem : '#table',
        url : '/management/config/exhibition/findByPagging',
        where: {
        },
        page: true,
        cols: [[
            {type:'numbers',title: '序号', width:160},
            {field:'name', sort: true, title: '会场', width:280},   
            {field:'addr', sort: false, title: '地址', width:320},
    ...
    $("#addBtn").on("click",function(){
        var index = layui.layer.open({
            title : '',
            type : 2,
            content : "/management/config/exhibition/add",
    ...
    layui.table.on('tool(table)', function(obj){
        ...
        if(layEvent === 'view'){
        	var index = layui.layer.open({
                title : '',
                type : 2,
                content : "/management/config/exhibition/view?exhibitionId="+obj.data.id,
                success : function(layero, index){
                    setTimeout(function(){
                        layui.layer.tips('点击此处返回会场库', '.layui-layer-setwin .layui-layer-close', {
                            tips: 3
        	...
        	var index = layui.layer.open({
                title : '',
                type : 2,
                content : "/management/config/exhibition/edit?exhibitionId="+obj.data.id,
                success : function(layero, index){
                    setTimeout(function(){
                        layui.layer.tips('点击此处返回会场库', '.layui-layer-setwin .layui-layer-close', {
                            tips: 3
...
function doDelete(exhibitionId){
	layer.confirm('确定要删除吗？', function(index){
	    ...
	    $.ajax({
            async: false,
            url: "/management/config/exhibition/deleteExhibition",
